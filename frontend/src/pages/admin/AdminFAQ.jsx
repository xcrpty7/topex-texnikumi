  ...
  return (
    <div style={{ color: '#272829' }}>
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-xl font-mono font-bold">Ko'p So'raladigan Savollar</h1>
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            <p className="font-mono text-xs" style={{ color: '#61677A' }}>{items.length} ta savol</p>
            {items.length > 0 && (
              <>
                ...
                {items.length - activeCount > 0 && (
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold" style={{ background: '#F1F2F4', color: '#61677A' }}>
                    {items.length - activeCount} yashirin
                  </span>
                )}
              </>
            )}
          ...
          {filtered.length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="font-mono text-sm" style={{ color: '#61677A' }}>
                {searchQ || visFilter !== 'all' ? 'Filtr bo\'yicha savol topilmadi' : 'Hali savol yo\'q'}
              </p>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)' }}>
          <div className="w-full max-w-lg rounded-xl p-6" style={{ background: '#FFFFFF', border: '1px solid #D8D9DA' }}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-mono font-bold" style={{ color: '#272829' }}>{editing ? 'Tahrirlash' : 'Savol qo\'shish'}</h2>
              <button onClick={() => setShowModal(false)}><X size={16} style={{ color: '#61677A' }} /></button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                <label className="block font-mono text-xs mb-1" style={{ color: '#61677A' }}>Savol</label>
              ...
              <div>
                <label className="block font-mono text-xs mb-1" style={{ color: '#61677A' }}>Javob</label>
      ...
      <ConfirmModal
        ...
        message="Bu savol va uning javobi butunlay o'chiriladi. Bu amalni qaytarib bo'lmaydi."
